function testTranslation(settingFileName,numTests)
    % input: the setting file name to test the translation on and the
    ...
    if isa(model,'char')
        modelname = model;
        [test_assessment_path, ~] = simConfig(modelname,input_param,hecateOpt);
    elseif isa(model,'function_handle')
        modelname = sys;
        [test_assessment_path, ~] = simConfig(modelname,input_param,hecateOpt,model);
    else
    ...
    hecateOpt = checkScenario(hecateOpt,test_assessment_path,false);

    % Build the corresponding Transition Map
    stepTableTA = getAllSteps(hecateOpt.assessment_scenario,test_assessment_path); 
    stepTableTA = sortAss(stepTableTA,[],[],hecateOpt.assessment_scenario); % Recursive function
    ...
    for ii = 1:numTests
        ...
        [robustnessStaliro, input_nodes, simData,fitnessSF] = simModel(model,hecateOpt.assessment_scenario);
...
function [robustnessStaliro,input_nodes,simResult,fitnessSF] = simModel(modelname,activeScenario,modelfcn)
    ...
    interpolation_fcn = hecateOpt.interpolationtype;      % Interpolation function for each input signal
                                                            % Options are:
